...
public final class TinyStorage: @unchecked Sendable {
	...
	public var allKeys: [any TinyStorageKey] {
		dispatchQueue.sync { Array(dictionaryRepresentation.keys) }
	}
	...
	public init(insideDirectory: URL, name: String) {
		let directoryURL = insideDirectory.appendingPathComponent(name)
		self.directoryURL = directoryURL

		let fileURL = directoryURL.appendingPathComponent("tiny-storage.plist")
	...
	private static func createEmptyStorageFile(directoryURL: URL, fileURL: URL, logger: Logger) -> Bool {
		...
		coordinator.coordinate(writingItemAt: directoryURL, options: [], error: &directoryCoordinatorError) { _ in
			do {
	...
	private static func retrieveStorageDictionary(directoryURL: URL, fileURL: URL, logger: Logger) -> [String: Data]? {
		...
		guard let storageDictionaryData else {
			return nil
		}
